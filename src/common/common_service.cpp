...
namespace buried {
	...
	static void WriteRegister(const std::string& key, const std::string& value) {
		...
		ret = ::RegSetValueExA(hKey, key.c_str(), 0, REG_SZ,
	...
	static std::string ReadRegister(const std::string& Key) {
		HKEY hKey;
		LONG ret = RegOpenKeyExA(HKEY_CURRENT_USER, "Software\\Buried", 0, KEY_ALL_ACCESS, &hKey);
		if (ret != ERROR_SUCCESS) {
			return "";
		}
		...
		ret = ::RegQueryValueExA(hKey, Key.c_str(), NULL, NULL, reinterpret_cast<BYTE*>(buf), &buf_size);
